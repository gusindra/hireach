<div>
    @if($model->status=='draft' || ($model->status=='unpaid' && $model_type == 'commission'))
        <div class="relative flex flex-col min-w-0 mt-6 mb-4 break-words bg-white border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-2xl bg-clip-border lg:mt-0">
            <div class="flex flex-wrap items-center mx-4 mt-4">
                <div class="w-9/12 max-w-full flex-0">
                    <h5 class="z-10 mb-1 text-transparent bg-clip-text bg-gradient-to-tl from-purple-700 to-pink-500">
                        <a href="javascript:;" class="text-transparent">Submission Progress</a>
                    </h5>
                </div>
            </div>
            <div class="relative flex flex-col min-w-0 mt-6 break-words bg-white border-0 dark:bg-gray-950 shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <div class="relative before:absolute before:left-4 before:top-0 before:h-full before:border-r-2 before:border-solid before:border-slate-100 before:content-[''] lg:before:-ml-px">
                    @foreach ($approvals as $approval)
                        <div class="relative mb-4">
                            ...
                            @elseif(in_array($approval->status, ['rejected', 'reject']))
                            ...
                            <div class="ml-12 pt-1.4 max-w-120 relative -top-1.5 w-auto">
                                <h6 class="mb-0 font-semibold leading-normal dark:text-white dark:text-slate-200 text-sm">{{$approval->status!='submited' ? $approval->role->name : $approval->user->name}} {{$approval->task}}</h6>
                                @if($approval->status)
                    ...
                    @if(@$approval->status!=null)
                        <div class="relative mb-4">
                            ...
                            <div class="ml-12 pt-1.4 max-w-120 relative -top-1.5 w-auto">
                                ...
                                @php $username = '' @endphp
                                @foreach ($approvals as $approval)
                                    @if($approval->user && $approval->user->name != $username)
                                    ...
                                    @endif
                                    @php $username = $approval->user->name @endphp
                                @endforeach
                            </div>
                        </div>
                    @endif
    ...
    @if($model->status=='submit' && $approval && $model->approval && checkRoles($model->approval->role_id))
        <div class="relative flex flex-col min-w-0 mt-6 mb-4 break-words bg-white border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-2xl bg-clip-border lg:mt-0">
            <div class="flex flex-wrap items-center mx-4 mt-4">
                <div class="w-9/12 max-w-full flex-0">
                    <h5 class="z-10 mb-1 text-transparent bg-clip-text bg-gradient-to-tl from-purple-700 to-pink-500">
                        <a href="javascript:;" class="text-transparent">Next Process</a>
                    </h5>
                </div>
            </div>
            <div class="px-4 py-5 sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                ...
                <div class="w-auto text-center mt-4 flex gap-2">
                    <button type="button" wire:click="decline" class="inline-block px-8 py-2 font-bold text-center uppercase align-middle transition-all bg-transparent border border-solid rounded-lg shadow-none cursor-pointer active:opacity-85 leading-pro text-xs ease-soft-in tracking-tight-soft bg-150 bg-x-25 hover:scale-102 active:shadow-soft-xs border-slate-700 dark:border-white dark:text-white dark:hover:text-white dark:active:border-white dark:hover:active:border-white dark:active:hover:text-white text-slate-700 hover:text-slate-700 hover:opacity-75 hover:shadow-none active:scale-100 active:border-slate-700 active:bg-slate-700 active:text-white hover:active:border-slate-700 hover:active:text-slate-700 hover:active:opacity-75">{{ __('Decline') }}</button>
                    <button wire:click="next" type="button" class="inline-block px-8 py-2 m-0 mb-0 font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer text-xs tracking-tight-soft ease-soft-in leading-pro bg-gradient-to-tl from-green-600 to-lime-400 shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85">{{ __('Approve') }}</button>
