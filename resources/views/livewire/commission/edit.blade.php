        ...
        <x-slot name="form">
            <div class="col-span-6 grid grid-cols-4">
                <div class="col-span-12 sm:col-span-1 mr-4">
                    ...
                    <select {{ disableInput($master->status) ? 'disabled' : '' }} name="clientId" id="clientId"
                ...
                <div class="col-span-12 sm:col-span-1 mr-4">
                    ...
                    <select {{ disableInput($master->status) ? 'disabled' : '' }} name="type" id="type"
                        class="border-gray-300 dark:bg-slate-800 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full"
                        wire:model="type" wire:model.debunce.800ms="type">
                        <option selected>-- Select --</option>
                ...
                <div class="col-span-12 sm:col-span-1 mr-4">
                    <div class="col-span-12 sm:col-span-1">
                        ...
                        <x-jet-input id="rate" disabled="{{ disableInput($master->status) }}" type="text"
                            class="mt-1 block w-full" wire:model="rate" wire:model.defer="rate"
                ...
                @if ($selected_agent && disableInput($master->status) != 1)
                    <div class="col-span-12 sm:col-span-1">
                        <a class="cursor-pointer mt-7 inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition"
                            wire:click="removeAgent()">
                            Clear
                        </a>
                    </div>
                @elseif($commission && $commission->status != 'draft')
                    <div class="col-span-12 sm:col-span-1 mr-4">
                        <div class="col-span-12 sm:col-span-1">
                            ...
                            <x-jet-input id="total" disabled="{{ disableInput($master->status) }}" type="text"
                                class="mt-1 block w-full" wire:model="total" wire:model.defer="total"
