    ...
    <div class="w-full">
        <div class="container max-w-4xl mx-auto">
            @if (session()->has('message'))
                <div class="flex items-center px-4 py-3 mb-6 text-sm font-bold text-white bg-green-500 rounded"
                    role="alert">
                    ...
                    @forelse($files as $file)
                        <tr>
                            ...
                            <td class="border border-gray-300 p-2">
                                <div><img src="{{ url('/backend/img/' . substr(strrchr($file->file, '.'), 1) . '.png') }}"
                                        class="h-8" title="{{ substr(strrchr($file->file, '/'), 1) }}" /></div>
                            </td>
                            <td class="border border-gray-300 p-2">
                                <div><a target="_blank"
                                        href="https://telixcel.s3.ap-southeast-1.amazonaws.com/{{ $file->file }}"
                                        title="download"><svg viewBox="0 0 34 34" width="34" height="34"
                                            class="ml-2">
            ...
            @if (!disableInput($status) || $status == 'unpaid' || $status == 'approved')
                <div class="mb-0 pb-6">
                    <form wire:submit.prevent="save" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <div wire:loading wire:target="input_file" class="text-sm italic text-gray-500">Uploading...
                            </div>
                        </div>
                        <button type="submit"
                            class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">Upload
                            File</button>
                    </form>
                </div>
            @endif
