    ...
    <x-jet-action-section>
        ...
        <x-slot name="content">
            <div class="flex items-center justify-end text-right">
                ...
                <x-save-button :disabled="!userAccess('SETTING', 'create')" show="true" wire:click="showCreateModal">
                    {{ __('Add Account') }}
                </x-save-button>
            </div>
                    ...
                    <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                            ...
                            @if (count($data['items']) > 0)
                                <div
                                    class="shadow overflow-hidden border-b border-gray-200 dark:border-slate-700 sm:rounded-lg">
                                    <table class="min-w-full divide-y divide-gray-200 mt-4">
                                        <thead>
                                            <tr>
                                                <th
                                                    ...
                                                    Method</th>
                                                <th
                                                    class="px-6 py-3 bg-gray-50 dark:bg-slate-800 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider w-11">
                                                    Provider</th>
                                                <th
                                        ...
                                        <tbody wire:poll class="bg-white dark:bg-slate-600 divide-y divide-gray-200">
                                            @foreach ($data['items'] as $item)
                                                <tr>
                                                    <td
                                                        class="px-6 text-xs py-4 whitespace-no-wrap uppercase align-top">
                                                        {{ $item->method }}
                                                    </td>
                                                    ...
                                                    <td class="items-center justify-end py-3 text-right align-top">
                                                        <div class="items-center flex">
                                                            <button title="{{ __('Delete') }}"
                                                                class="cursor-pointer m-2 w-auto text-sm p-1 bg-red-200 text-gray-800 rounded"
    ...
    <x-jet-dialog-modal wire:model="modalVisible">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4 p-3">
                ...
                <select name="input.method" id="input.method"
            ...
            <div class="col-span-6 sm:col-span-4 p-3 grid grid-cols-3">
                <div class=" ">
                    ...
                    <x-jet-input id="input.provider_name" type="text" class="mt-1 block w-full"
                        wire:model.debunce.800ms="input.provider_name" autofocus />
                    <x-jet-input-error for="input.provider_name" class="mt-2" />
                </div>
                <div class="mx-3">
                    <x-jet-label for="input.account_number" value="{{ __('Account Number') }}" />
                    <x-jet-input id="input.account_number" type="text" class="mt-1 block w-full"
                        wire:model.debunce.800ms="input.account_number" autofocus />
                    <x-jet-input-error for="input.account_number" class="mt-2" />
                </div>
                <div class="">
                    <x-jet-label for="input.account_name" value="{{ __('Account Name') }}" />
                    <x-jet-input id="input.account_name" type="text" class="mt-1 block w-full"
                        wire:model.debunce.800ms="input.account_name" autofocus />
                    <x-jet-input-error for="input.account_name" class="mt-2" />
                </div>
            </div>
            <div class="col-span-6 sm:col-span-4 p-3">
                <x-jet-label for="input.provider_location" value="{{ __('Provider Location') }}" />
                <x-jet-input id="input.provider_location" type="text" class="mt-1 block w-full"
                    wire:model.debunce.800ms="input.provider_location" autofocus />
                <x-jet-input-error for="input.provider_location" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4 p-3">
                <x-jet-label for="input.notes" value="{{ __('Notes') }}" />
                <x-jet-input id="input.notes" type="text" class="mt-1 block w-full"
                    wire:model.debunce.800ms="input.notes" autofocus />
