<div>
            ...
            <div class="flex items-center justify-between flex-wrap">
                <div class="w-0 flex-1 flex items-center">
                    @if ($client)
                        ...
                        <div
                            class="md:h-auto text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition pr-3 mt-1">
                            <img class="h-7 w-7 rounded-full object-cover" src="{{ $client->profile_photo_url }}"
                                alt="{{ $client->name }}" />
                        </div>
                ...
                @if (@$handling_session)
                    @if ($handling_session->agent_id == $user_id && $handling_session->client_id == $client_id)
                        <button class="mr-2 p-1 border-transparent bg-red-500 hover:bg-red-800 text-white"
                            title="Close Session" wire:click="showModalConfirmation">
    ...
    <div id="messageArea" class="h-1/2">
        @if (
            @$handling_session &&
                $handling_session->agent_id == $user_id &&
                $handling_session->client_id == $client_id &&
                $data['count'] > 2)
            ...
            @endif
            <p
                class="text-center dark:bg-slate-300 {{ $transcript ? 'bg-gray-600 text-gray-100' : 'bg-gray-200 text-gray-800' }}">
                <a class="text-xs p-4 underline" href="#"
                    wire:click="showTransript">{{ $transcript ? 'Show less' : 'See transcript' }}</a>
            </p>
        @endif
        <div id="messageBox" wire:poll.visible class="overflow-auto h-screen bg-green-50 dark:bg-slate-700 py-0"
            style="display: flex;flex-direction: column;;overflow: auto;">
            @foreach ($data['request'] as $item)
                <div
                    class="px-4 sm:px-4 buble-chat object-left group {{ preg_match('/[a-z]/i', $item->source_id) ? '' : 'text-right1 right-0' }}">
                    <p class="{{ preg_match('/[a-z]/i', $item->source_id) ? '' : 'text-right right-0' }}"><small
                            class="text-gray-500 dark:text-slate-300 font-medium">{{ preg_match('/[a-z]/i', $item->source_id) ? $client?->name ?? ($client?->email ?? $client?->phone) : ($item->from == 'bot' || $item->from == 'api' ? 'Bot' : $item->agent?->name ?? $item->agent?->email) }}
                    ...
                    <div class="flex justify-between">
                        <div
                            class="text-sm flex-auto z-9 p-2 xl:p-3 bg-gradient-to-br {{ preg_match('/[a-z]/i', $item->source_id) ? 'items-start' : 'order-last text-right1' }} {{ preg_match('/[a-z]/i', $item->source_id) ? 'from-green-100 to-green-200 rounded-tr-lg rounded-b-lg' : 'from-indigo-100 to-indigo-200 rounded-b-lg rounded-tl-lg right-0' }}">
                            @if ($item->type == 'image')
                            ...
                            @endif
                            <div
                                class="{{ $item->source_id ? ($item->type !== 'text' ? 'rounded-tr-lg rounded-b-lg' : '') : ($item->type !== 'text' ? 'from-indigo-100 to-indigo-200 rounded-sm right-0 p-4' : '') }} mb-1 flex items-stretch justify-between w-100 whitespace-pre-wrap">
                                <div class="flex flex-row">
                                    @if ($item->type == 'document')
                                        <img src="{{ url('/backend/img/' . substr(strrchr($item->media, '.'), 1) . '.png') }}"
                                            class="h-8" />
                                    @endif
                                    <span
                                        class="{{ $item->type == 'document' ? 'ml-2 mt-2' : '' }} dark:text-slate-600">{!! chatRender($item) !!}</span>
                                </div>
                                @if ($item->media != '')
                                    @if ($item->type == 'document')
                                        <a href="{{ $item->media }}" data-testid="audio-download"
                                            data-icon="audio-download" class=""><svg viewBox="0 0 34 34"
                            ...
                            <p
                                class="px-3 text-xs font-thin text-gray-400 {{ $item->source_id ? 'text-right right-0' : 'text-left left-0' }} dark:text-slate-500">
                                {{ $item->date }}</p>
                                ...
                                @if ($handling_session->agent_id == $user_id && $handling_session->client_id == $client_id)
                                    @if ($item->activeTickets->count() == 0 && $item->source_id)
                                        <section class="max-w-sm mx-auto rounded-lg flex flex-row items-center ml-2">
                                            <aside
                                                class="invisible group-hover:visible h-full text-center flex-grow flex flex-col text-sm ml-auto divide-y relative rounded-md">
                                                <a class="text-blue-500 hover:text-blue-700 h-1/2 flex items-center text-left text-xs dark:bg-slate-800 w-1/2 p-1"
                                                    href="#"
                                                    wire:click="ticketShowModal({{ $item->id }}, '{{ $item->reply }}')">Convert
                                                    Ticket</a>
                                                <a class="text-blue-500 hover:text-blue-700 h-1/2 flex items-center text-left text-xs dark:bg-slate-800 w-1/2 p-1"
                                                    href="#"
                                                    wire:click="forwardShowModal({{ $item->id }}, '{{ $item->reply }}')">Forward</a>
                    ...
                    <div class="w-100 sticky top-0 z-9 bottom-0">
                        @foreach ($item->tickets as $ticket)
                            @if (@$handling_session)
                                @if ($handling_session->agent_id == $user_id && $handling_session->client_id == $client_id)
                                    @if ($ticket->status == 'open')
                                        <a class="mt-4 flex justify-center gap-x-2 text-xs text-gray-100 px-10 py-1 mb-0 bg-red-600 hover:bg-red-800 cursor-pointer"
                                            wire:click="ticketUpdateModal({{ $ticket->request_id }}, {{ $ticket->id }}, '{{ $ticket->reasons }}', '{{ $ticket->status }}')">
                                    ...
                                    @elseif($ticket->status == 'waiting')
                                        <a class="mt-4 flex justify-center gap-x-2 text-xs text-gray-100 px-10 py-1 mb-4 bg-blue-600 hover:bg-blue-800 cursor-pointer"
                                            wire:click="ticketUpdateModal({{ $ticket->request_id }}, {{ $ticket->id }}, '{{ $ticket->reasons }}', '{{ $ticket->status }}', '{{ $ticket->forward_to }}')">
                            ...
                            @else
                                @if ($ticket->status == 'open')
                                    <a
                                        class="mt-4 flex justify-center gap-x-2 text-xs text-gray-100 px-10 py-1 mb-0 bg-red-600">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
                                ...
                                @elseif($ticket->status == 'waiting')
                                    <a
                                        class="mt-4 flex justify-center gap-x-2 text-xs text-gray-100 px-10 py-1 mb-4 bg-blue-600">
        ...
        @if ($client)
            <div
                class="{{ count($data['quick']) == 0 ? 'hidden' : '' }} lg:absolute lg:bottom-24 bottom-1 mb-16 lg:w-2/4 w-100">
                    ...
                    <ul
                        ...
                        @foreach ($data['quick'] as $quick)
                            <li role="option"
                                ...
                                <button wire:click="showQuickModal({{ $quick->id }})"
                                    class="py-2 pl-3 text-xs text-gray-900 cursor-default select-none pr-9 w-full text-left focus:text-white focus:bg-indigo-600 hover:text-white hover:bg-indigo-600">
                                    {{ $quick->name }} :
            ...
            <div
                class="py-3 grid grid-cols-8 z-10 md:static sm:fixed sm:inset-x-0 sm:bottom-0 lg:fixed lg:bottom-0 dark:bg-slate-800">
                @if (@$handling_session)
                    @if ($handling_session->agent_id == $user_id && $handling_session->client_id == $client_id)
                        ...
                        <div data-emojiarea data-type="unicode" data-global-picker="false"
                            ...
                            <x-textarea id="message" class="mt-1 block w-full h-full text-sm"
                                placeholder="{{ __('write a reply...') }}" wire:model="message" />
                        </div>
                        <div class="flex items-center justify-center col-span-1 align-text-bottom">
                            <button class="p-2 sm:p-2 md:p-4 lg:p-4 bg-green-600 text-white rounded-full"
                    ...
                    @elseif($handling_session->agent_id == $user_id)
                        <div class="flex w-full items-center justify-center col-span-12 my-6 text-sm text-gray-400">
                            <p class="px-4">You still have active conversation. Please end last chat before join new
                                conversation</p>
                        </div>
                    @else
                        <div class="flex w-full items-center justify-center col-span-12 my-6 text-sm text-gray-400">
                ...
                @else
                    ...
                    <div class="justify-center w-100 flex col-span-12 mt-3">
                        <button class="bg-green-600 text-white border border-gray-300 px-6 py-2 mb-8 rounded-md"
                            wire:click="joinChat">
                            {{ __('Join') }}
                        </button>
                    </div>
    ...
    <x-jet-dialog-modal wire:model="modalAttachment">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4 p-3">
                <div x-data="{ photoName: null, photoPreview: null }" class="col-span-6 sm:col-span-4">
                    ...
                    <div class="mt-2" x-show="photoPreview">
                        <span class="block h-48 w-full"
                            x-bind:style="'background-size: contain; background-repeat: no-repeat; background-position: center center; background-image: url(\'' +
                            photoPreview + '\');'">
                        </span>
                    </div>
                ...
                <x-textarea wire:model="message" wire:model.defer="message" value="message"
                    class="mt-1 block w-full" placeholder="Caption"></x-textarea>
    ...
    <x-jet-dialog-modal wire:model="modalTicket">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4 p-3">
                ...
                <x-textarea wire:model="reason" wire:model.defer="reason" value="reason" class="mt-1 block w-full"
                    placeholder="Reason"></x-textarea>
                <x-jet-input-error for="name" class="mt-2" />
            </div>
        </x-slot>
    ...
    <x-jet-dialog-modal wire:model="modalUpdateTicket">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4 p-3">
                <label for="no">Ticket No <input class="text-center" wire:model="request_id"
                        wire:model.defer="request_id" value="request_id" readonly /></label>
                ...
                @if ($ticket_type == null)
                    ...
                    <x-textarea wire:model="ticket_solution" wire:model.defer="ticket_solution"
