                                            ...
                                            <tbody class="bg-white dark:bg-slate-700 divide-y divide-gray-200">
                                                @foreach ($data as $item)
                                                    <tr>
                                                        ...
                                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                                            <a wire:click="$emit('actionSelected',{{ $item->id }})"
                                                                href="{{ route('show.template', $item->uuid) }}"
                                                                class="underline p-2 border border-transparent text-xs font-medium rounded-md {{ $item->is_enabled == 1 ? 'text-indigo-700 bg-indigo-200 hover:bg-indigo-200' : 'text-red-700 bg-red-100 hover:bg-red-200' }}">
                                                                {{ $item->name }}
                                                            </a>
                                                        </td>
                                                        <td
    ...
    <x-jet-dialog-modal wire:model="modalCreateVisible">
        ...
        <x-slot name="content">
            ...
            <div class="col-span-6 sm:col-span-4 p-3">
                ...
                <x-jet-input id="trigger" type="text" class="mt-1 block w-full" wire:model.debunce.800ms="trigger"
