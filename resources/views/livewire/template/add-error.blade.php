                                            ...
                                            <tbody class="bg-white dark:bg-slate-700 divide-y divide-gray-200">
                                                <tr>
                                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                                        <a href="{{ route('show.template', $errorUuid) }}"
                                                            class="underline p-2 border border-transparent text-xs font-medium rounded-md {{ $errorEnabled == 1 ? 'text-indigo-700 bg-indigo-200 hover:bg-indigo-200' : 'text-red-700 bg-red-100 hover:bg-red-200' }}">
                                                            {{ $errorName }}
                                                        </a>
                                                    </td>
                                                    <td
                                                        ...
                                                        <div class="flex items-center">
                                                            <button class="cursor-pointer ml-6 text-sm text-red-500"
                                                                wire:click="deleteShowModal('{{ $errorId }}')">
                                                                {{ __('Remove') }}
                                                            </button>
                                                        </div>
                                                    </td>
                    ...
                    <div class="col-span-6 sm:col-span-6">
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="same_template" name="same_template" wire:model="sameTemplate"
                                    wire:click="setDefaultError" wire:model.defer="sameTemplate" type="checkbox"
                                    class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                            </div>
                            <div class="ml-3 text-sm">
                                <label for="same_template" class="font-medium text-gray-700 dark:text-slate-300">Resent
                                    Action Message</label>
                                <p class="text-gray-500 dark:text-slate-300">Enable to resend Action Message if error /
                                    wrong answer.</p>
                            </div>
                        </div>
                    </div>
