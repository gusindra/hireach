<div>
                                                ...
                                                @foreach ($data as $item)
                                                    <tr>
                                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                                            <div
                                                                class="w-full flex md:flex-col bg-gradient-to-br from-green-100 to-green-200 rounded-tr-2xl rounded-b-xl">
                                                                <div class="items-start relative z-10 p-3 xl:p-3">
                                                                    ...
                                                                    @elseif($item->type == 'document')
                                                                        <div class="flex justify-between">
                                                                            <div><img
                                                                                    src="{{ url('/backend/img/' . substr(strrchr($item->message, '.'), 1) . '.png') }}"
                                                                                    class="h-8" /></div>
                                                                            <div class="text-left mt-1">
                                                                                {{ substr(strrchr($item->message, '/'), 1) }}
                                                                            </div>
                                                                            <div><a href="{{ $item->message }}"
                                                                    ...
                                                                    @else
                                                                        <span
                                                                            class="whitespace-pre-wrap dark:text-slate-600">{{ $item->message }}</span>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                        </td>
    ...
    <x-jet-dialog-modal wire:model="modalActionVisible">
        ...
        <x-slot name="content">
            ...
            @if ($content == 'text')
                <div>
                    <div class="col-span-6 sm:col-span-4 p-3">
                        ...
                        <x-textarea role="textbox" contenteditable wire:model="message" wire:model.defer="message"
                            value="message" class="mt-1 block w-full" :disabled="!Gate::check('update', $template)"
            ...
            @elseif($content == 'file')
                <div>
                    <div class="col-span-6 sm:col-span-4 p-3"><x-jet-label for="photo"
                            value="{{ __('Link') }}" />
