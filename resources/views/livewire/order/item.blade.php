        ...
        <x-slot name="content">
            <div class="flex items-center justify-end text-right">
                ...
                @if ($data->status == 'draft')
                    <x-add-button :disabled="!userAccess('ORDER', 'add')" :show="$data->status == 'draft'" wire:click="showCreateModal">
                        {{ __('Add Item') }}
                    </x-add-button>
                @endif
                                ...
                                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                    <table class="min-w-full divide-y divide-gray-200 mt-2">
                                        <thead>
                                            <tr>
                                                <th
                                                    class="px-6 py-3 bg-gray-50 dark:bg-slate-700 dark:text-slate-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider w-1/4">
                                                    Item</th>
                                                <th
                                                    class="px-3 py-3 bg-gray-50 dark:bg-slate-700 dark:text-slate-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider w-11">
                                                    Price</th>
                                                <th
                                                    class="px-6 py-3 bg-gray-50 dark:bg-slate-700 dark:text-slate-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider w-1/4">
                                                    Qty</th>
                                                <th
                                                    class="px-6 py-3 bg-gray-50 dark:bg-slate-700 dark:text-slate-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider w-1/2">
                                                </th>
                                                <th
                                                    class="px-6 py-3 bg-gray-50 dark:bg-slate-700 dark:text-slate-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider w-1/4">
                                                    Amount</th>
                                        ...
                                        <tbody wire:poll.visible
                                            class="bg-white dark:bg-slate-600 divide-y divide-gray-200">
                                            @foreach ($data['items'] as $item)
                                                <tr>
                                                    ...
                                                    <td
                                                        class="items-center justify-end py-3 text-right {{ $data->status == 'draft' ? '' : 'hidden' }}">
                                                        <div class="items-center flex">
                                                            <button title="{{ __('Delete') }}"
                                                                class="cursor-pointer m-2 w-auto text-xs p-1 bg-red-200 text-gray-800 rounded"
                                ...
                                <div class="mt-10">
                                    ...
                                    <div class="col-span-6 grid grid-cols-4 mt-4">
                                        <div class="col-span-4 sm:col-span-1 mx-4">
                                            ...
                                            <x-jet-input id="name" wire:change="updateTax"
                                                disabled="{{ disableInput($data->status) }}" type="text"
                                                class="mt-1 block w-full text-right" wire:model="tax"
                                                wire:model.defer="tax" wire:model.debunce.800ms="tax" />
    ...
    <x-jet-dialog-modal wire:model="modalVisible">
        ...
        <x-slot name="content">
            ...
            <div class="col-span-6 sm:col-span-4 p-3 grid grid-cols-3">
                ...
                <div class="ml-3">
                    ...
                    <x-jet-input id="qty" type="text" class="mt-1 block w-full"
                        wire:model.debunce.800ms="qty" autofocus />
                    <x-jet-input-error for="qty" class="mt-2" />
                </div>
                <div class="ml-1">
                    <x-jet-label for="unit" value="{{ __('Unit Measurement') }}" />
                    <x-jet-input id="unit" placeholder="meter, unit, item, dll" type="text"
                        class="mt-1 block w-full" wire:model.debunce.800ms="unit" autofocus />
            ...
            <div class="col-span-6 sm:col-span-1 p-3">
                ...
                <div class="flex">
                    <x-jet-input id="name" type="text" class="mt-1 block w-full text-right"
                        wire:model.debunce.800ms="percentage" autofocus />
                    <span class="p-3 bordered border-1">%</span>
                </div>
                <span class="text-xs">Default 100% of total. You can set for commission or percentage after
                    discount.</span>
                <x-jet-input-error for="name" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4 p-3">
            ...
            @if ($products)
                ...
                <div class="col-span-6 sm:col-span-4 p-3">
                    ...
                    <x-jet-input id="unit" type="text" class="mt-1 block w-full"
                        wire:model.debunce.800ms="unit" autofocus />
                    <x-jet-input-error for="unit" class="mt-2" />
                </div>
                <div class="col-span-6 sm:col-span-4 p-3">
                    <x-jet-label for="description" value="{{ __('Description') }}" />
