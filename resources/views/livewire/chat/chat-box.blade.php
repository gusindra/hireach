<div>
    <div
        class="bg-gray-400 dark:bg-slate-700 sm:h-12 lg:h-14 lg:static md:static sm:fixed sm:inset-x-0 sm:top-0 shadow-md">
                ...
                <div class="w-0 flex-1 flex items-center">
                    @if ($team && $client)
                        ...
                        <div
                            ...
                            <img class="h-8 w-8 rounded-full object-cover" src="{{ $team->profile_photo_url }}"
                                alt="{{ $team->name }}" />
                        </div>
    ...
    <div id="messageArea" class="lg:max-h-screen">
        <div id="messageBox" wire:poll class="overflow-auto h-96 bg-green-50 dark:bg-slate-600 py-4"
            style="display: flex;flex-direction: column;height: 80vh;overflow: auto;">
            @foreach ($data as $item)
                <div
                    class="pb-1 px-4 sm:p-2 sm:px-3 buble-chat object-left {{ $item->source_id ? '' : 'text-right right-0' }}">
                    <small
                        class="text-gray-500 dark:text-slate-300 font-medium">{{ $item->source_id ? $client->name : ($item->from == 'bot' || $item->from == 'api' ? 'Bot' : $item->agent->name) }}</small>
                    <div class="flex justify-between">
                        <div
                            class="text-sm flex-auto z-10 p-2 xl:p-3 bg-gradient-to-br {{ $item->source_id ? 'items-start' : 'order-last text-right' }} {{ $item->source_id ? 'from-green-100 to-green-200 rounded-tr-lg rounded-b-lg' : 'from-indigo-100 to-indigo-200 dark:bg-slate-700 rounded-b-lg rounded-tl-lg right-0' }}">
                            @if ($item->type == 'image')
                                <img src="{{ $item->media }}"
                            ...
                            @else
                                <span
                                    class="whitespace-pre-wrap dark:text-slate-600 text-base">{{ $item->reply }}</span><br>
                            @endif
                            <p
        ...
        @if ($client)
            <div
                class="bg-gray-200 dark:bg-slate-700 py-1 grid grid-cols-6 lg:static md:static sm:fixed sm:inset-x-0 sm:bottom-0">
                <div class="flex items-center justify-center col-span-1 align-text-bottom">
                    <button class="cursor-pointer text-sm text-grey-500 dark:text-slate-300 p-2"
                ...
                <div class="md:flex pl-2 py-2 col-span-4">
                    <x-textarea id="message" class="mt-1 block w-full" placeholder="{{ __('write a reply...') }}"
                        wire:model="message" />
                </div>
                <div class="flex items-center justify-center col-span-1 align-text-bottom">
                    <button class="p-2 sm:p-2 md:p-4 lg:p-4 bg-green-600 text-white rounded-full"
                        wire:click="sendMessage">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 rotate-30" viewBox="0 0 20 20"
                            fill="currentColor">
    ...
    <x-jet-dialog-modal wire:model="modalAttachment">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4 p-3">
                ...
                <x-textarea wire:model="message" wire:model.defer="message" value="message" class="mt-1 block w-full"
                    placeholder="Caption"></x-textarea>
