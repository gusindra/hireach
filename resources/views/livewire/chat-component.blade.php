<div>
    ...
    <div class="max-w-full sm:px-1 lg:px-1 lg:mr-6 sm:ml-24">
        <div class="lg:grid lg:grid-cols-12 gap-2 lg:h-screen">
            <!-- List Customer -->
            <div
                class="col-start-1 bg-white dark:bg-slate-600 mt-2 overflow-hidden shadow-xl sm:rounded-sm col-span-2 lg:h-screen lg:max-h-full">
                            ...
                            <div class="w-0 flex-1 flex items-center">
                                <div class="form-check mx-2 mt-2">
                                    <label class="form-check-label ps-2 text-sm " for="active">
                                        <input class="hidden" value="active" type="radio" name="filter"
                                            id="active" wire:model="filter">
                                        <div class="z-10 relative text-white label-checked:active">{{ __('Active') }}
                                        </div>
                                    </label>
                                </div>
                                <div class="form-check mx-2 mt-2">
                                    <label class="form-check-label ps-2 text-sm" for="waiting">
                                        <input class="hidden" value="waiting" type="radio" name="filter"
                                            id="waiting" wire:model="filter">
                                        <div class="z-10 relative text-white label-checked:active">{{ __('Waiting') }}
                                        </div>
                                    </label>
                                </div>
                                <div class="form-check mx-2 mt-2">
                                    <label class="form-check-label ps-2 py-1 text-sm" for="handled">
                                        <input checked class="hidden" value="handled" type="radio" name="filter"
                                            id="handled" wire:model="filter">
                                        <div class="z-10 relative text-white label-checked:active">{{ __('Handled') }}
                                        </div>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div wire:poll.keep-alive>
                        ...
                        @foreach ($tickets as $ticket)
                            <a x-on:click="window.scrollBy(0, $refs.blue.getBoundingClientRect().top - 50"
                                wire:click="chatCustomer('{{ Hashids::encode($ticket->request->client->id) }}')"
                                class="cursor-pointer client-click">
                                <div class="text-sm whitespace-no-wrap border-l-8 border-red-600">
                                    <div
                                        class="md:flex px-2 py-1 hover:bg-gray-200 border-b-2 border-gray-400 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-gray-100 focus:ring-opacity-50">
                                        <div
                                            class="sm:max-w-sm sm:flex-none md:w-auto md:flex-auto flex-col items-start relative z-10 p-1">
                                            <div class="flex">
                                                <div class="flex-grow">Ticket
                                                    #{{ $ticket->request_id }}<br><small>{{ $ticket->reasons }}</small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        @foreach ($data as $item)
                            <a x-on:click="window.scrollBy(0, $refs.blue.getBoundingClientRect().top - 50"
                                wire:click="chatCustomer('{{ Hashids::encode($item->id) }}')"
                                class="cursor-pointer client-click">
                                <div
                                    class="text-sm whitespace-no-wrap {{ auth()->user()->chatsession && auth()->user()->chatsession->client_id == $item->id ? 'border-l-8 border-green-600' : '' }}">
                                    <div
                                        class="md:flex px-2 py-2 {{ $client_id != 0 && $client_id == $item->id ? 'bg-gray-300 dark:bg-slate-800' : 'bg-white dark:bg-slate-700' }} hover:bg-gray-200 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-gray-100 focus:ring-opacity-50">
                                        <div
                                            class="md:h-auto text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                            <img class="h-8 w-8 rounded-full object-cover"
                                                src="{{ $item->profile_photo_url }}" alt="{{ $item->name }}" />
                                        </div>
                                        <div
                                            class="sm:max-w-sm sm:flex-none md:w-auto md:flex-auto flex-col items-start relative z-10 p-2 xl:p-2">
                                            <div class="flex">
                                                <div class="flex-grow">
                                                ...
                                                @if ($item->active && countMsg($item) > 0)
                                                    <div class="flex w-auto text-center">
                                                        <div
                                                            class="px-1 w-auto h-4 m-1 rounded-full bg-red-500 text-xs text-gray-50 text-center">
                                                            {{ countMsg($item) }}</div>
                                                    </div>
                                                @endif
                                            </div>
            ...
            <!-- Area Chatting -->
            <div x-ref="blue" id="chatArea"
                class="bg-gray-200 dark:bg-slate-600 overflow-hidden shadow-xl mt-2 sm:rounded-sm col-span-7 bg-blend-darken h-auto">
            ...
            @if ($client)
                <div
                    class="hidden md:inline bg-white dark:bg-slate-600 overflow-hidden shadow-xl sm:rounded-sm col-span-2 mt-2">
                    <div class="p-1">
                        ...
                        <div class="p-1">
                            <input class="border-gray-300 focus:border-indigo-300 w-full my-1 text-xs dark:bg-slate-700"
                                type="text" placeholder="{{ __('Customer Phone') }}" x-ref="phone"
                                wire:model.defer="client_phone" readonly disabled>
                            <input class="border-gray-300 focus:border-indigo-300 w-full my-1 text-xs dark:bg-slate-700"
                                type="text" placeholder="{{ __('Customer Name') }}" x-ref="client_name"
                            ...
                            <textarea class="border-gray-300 focus:border-indigo-300 w-full my-1 text-xs dark:bg-slate-700" id="note"
                                class="mt-1 block w-full mb-4" placeholder="{{ __('write note here') }}" wire:model.defer="note" wire:model="note"></textarea>
                            <input class="border-gray-300 focus:border-indigo-300 w-full text-xs dark:bg-slate-700"
                                type="text" placeholder="{{ __('Add tag here') }}" x-ref="tag"
                                wire:model.defer="tag">
                            <div class="my-1 text-right">
                                <button class="bg-gray-100 border border-gray-300 px-4 py-1 dark:bg-slate-700"
                                    wire:click="save">
                                    {{ __('Save') }}
                                </button>
            ...
            <div class="{{ $client ? 'block' : 'hidden' }}">
                <div
                    class="{{ count($dataTemplate['quick']) == 0 ? 'hidden' : '' }} lg:absolute lg:bottom-24 lg:mb-10 lg:left-0 lg:ml-32 bottom-1 lg:w-2/4 w-100">
                    <div
                ...
                @if ($handlingSession && $client && $client_id)
                    <div
                        class="{{ $handlingSession->client_id != $client_id ? 'block' : 'hidden' }} py-10 z-10 md:static sm:fixed sm:inset-x-0 sm:bottom-0 lg:fixed lg:bottom-0 dark:bg-yellow-800">
                        <div id="msg_session">
                ...
                @endif
                <div id="texting-area"
                    class="{{ $handlingSession && $handlingSession->client_id == $client_id ? 'block' : 'hidden' }} pb-2 grid grid-cols-12 z-10 md:static sm:fixed sm:inset-x-0 sm:bottom-0 lg:fixed lg:bottom-0 dark:bg-slate-800 bg-gray-50">
                    <div
                        class="flex flex-row-reverse items-center col-span-1 align-text-bottom emoji-button cursor-pointer text-sm text-grey-500 dark:text-slate-300 p-1">
                    ...
                    <div data-emojiarea data-type="unicode" data-global-picker="false"
                        class="flex py-2 col-span-10 pr-2">
                        <div
                    ...
                    <div class="flex items-center justify-center align-text-bottom">
                        <button
                            class="w-1/2 p-2 sm:p-2 md:p-4 lg:p-4 bg-green-600 text-white rounded-full text-center items-center flex justify-center"
                ...
                <div
                    class="{{ $handlingSession ? 'hidden' : 'block' }} hidden z-10 md:static sm:fixed sm:inset-x-0 sm:bottom-0 lg:fixed lg:bottom-0 dark:bg-slate-800">
        ...
        @push('chat-websocket')
            <script>
                $(function() {
                    ...
                    var chatConnection = clientSocket({
                        port: 3281
                    });
