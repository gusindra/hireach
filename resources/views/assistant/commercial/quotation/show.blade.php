                ...
                <div class="justify-end flex">
                    @if (!$quote->order)
                        @can('create', App\Models\Order::class)
                            @livewire('order.quotation-to-order', ['id' => $quote->id])
                        @endcan
                    @else
                        <span class="inline-flex rounded-md mb-2">
                            <a href="{{ route('show.order', $quote->order->id) }}"
                    ...
                    <div class="items-center justify-end px-2 text-right">
                        <x-jet-dropdown align="right" width="60">
                            <x-slot name="trigger">
                                <span class="inline-flex rounded-md mb-2">
                                    <button type="button"
                                        class="inline-flex dark:bg-slate-800 dark:text-slate-300 items-center px-3 py-2 border text-xs leading-4 font-medium rounded-md text-gray-500  bg-gray-200 hover:bg-gray-300 hover:text-gray-700 focus:outline-none focus:bg-gray-400 active:bg-gray-400 transition">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20"
                                            fill="currentColor">
                                            <path
                                    ...
                                    <div>
                                        @if (count($quote->attachments) > 0)
                                            @foreach ($quote->attachments as $file)
                                                <a title="Quotation upload at {{ $file->created_at->format('d F Y - H:i') }}"
                                                    class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition"
                                                    target="_blank"
                                                    href="https://telixcel.s3.ap-southeast-1.amazonaws.com/{{ $file->file }}">
                                                    <div class="flex items-center justify-between">
                                                        <div class="truncate text-xs">Download Quotation
                                                            {{ $loop->iteration }}</div>
                                                    </div>
                                                </a>
                                            @endforeach
                                        @else
                                            <a class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition"
                                                target="_blank"
                                                href="{{ route('commercial.print', ['type' => 'quotation', 'id' => $quote->id]) }}">
                                                <div class="flex items-center justify-between">
        ...
        <div class="max-w-8xl mx-auto pt-4 pb-10 sm:px-6 lg:px-8 lg:ml-20">
                ...
                <div class="md:grid md:grid-cols-5 md:gap-4">
                    ...
                    <div class="mt-0 md:mt-0 md:col-span-4">
                        <div class="bg-white dark:bg-slate-600 shadow sm:rounded-md">
                            <div class="pt-4">
                                @livewire('file-upload', ['model' => 'quotation', 'model_id' => $quote->id, 'status' => $quote->status])
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="md:grid md:grid-cols-5 md:gap-6 mt-8 sm:mt-0">
                <div class="md:col-span-4">
                    @livewire('commercial.quotation.edit', ['code' => @$quote->id, 'source' => app('request')->input('source'), 'source_id' => app('request')->input('id')])
                </div>
                <div class="col-span-1">
                    <div class="justify-between lg:visible md:invisible">
                        @livewire('commercial.progress', ['model' => 'quotation', 'id' => $quote->id])
                    </div>
                </div>
            </div>
        </div>
