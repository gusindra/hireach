...
* Product Page: product soft-tailwind
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-cloak x-init="$watch('darkMode', (val) => localStorage.setItem('dark',val))" x-data="{darkMode: localStorage.getItem('dark')}" :class="darkMode==='true' || darkMode==true ? 'dark' : ''" :data-dark="darkMode">
    ...
    <body class="m-0 font-sans antialiased font-normal text-left leading-default text-base dark:bg-slate-950 bg-gray-50 text-slate-500 dark:text-white/80">
        ...
        <div fixed-plugin>
            ...
            <div fixed-plugin-card class="dark:bg-gray-950/80 z-sticky shadow-soft-3xl w-90 ease-soft -right-90 fixed top-0 left-auto flex h-full min-w-0 flex-col break-words rounded-none border-0 bg-white/80 bg-clip-border px-2.5 backdrop-blur-2xl backdrop-saturate-200 duration-200">
                ...
                <div class="flex-auto p-6 pt-0 sm:pt-4">
                    ...
                    <div class="min-h-6 mb-0.5 block pl-0">
                        <input x-on:click="darkMode==='true' || darkMode==true ? darkMode=false : darkMode=true;" id="dark-toggle" dark-toggle class="rounded-10 duration-250 ease-soft-in-out after:rounded-circle after:shadow-soft-2xl after:duration-250 checked:after:translate-x-5.3 h-5 relative float-left mt-1 ml-auto w-10 cursor-pointer appearance-none border border-solid border-gray-200 bg-slate-800/10 bg-none bg-contain bg-left bg-no-repeat align-top transition-all after:absolute after:top-px after:h-4 after:w-4 after:translate-x-px after:bg-white after:content-[''] checked:border-slate-800/95 checked:bg-slate-800/95 checked:bg-none checked:bg-right" type="checkbox" />
                    </div>
                    ...
                    <div class="flex text-sm absolute bottom-0 pb-2">
                        <a href="https://www.telixcel.com" class="font-semibold text-slate-700 dark:text-white" target="_blank">Telixcel <span class="mx-1">©</span></a>
                        <script>
        ...
        <script src="{{ url('js/soft-tailwind.min.js') }}?v=1.0.1"></script>
